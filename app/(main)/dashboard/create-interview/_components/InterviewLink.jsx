...
import { ArrowDown, Copy, Clock, List, Calendar, Mail, ArrowLeft, Plus, MessageCircle, Smartphone } from 'lucide-react';
...
function InterviewLink({ interview_id, formData }) {
    ...
    const jobTitle = formData?.jobPosition || "your interview";
    const shareSubject = `IntelliHire interview link for ${jobTitle}`;
    const shareMessage = `Hi, your AI interview for ${jobTitle} is ready. Please open this link to start: ${final_url}`;
    const getUrl = () => {
        return final_url
    }
    ...
    const onEmailShare = () => {
        const url = `mailto:?subject=${encodeURIComponent(shareSubject)}&body=${encodeURIComponent(shareMessage)}`;
    ...
    return (
        ...
        <div className='flex flex-col items-center bg-white !px-6 !py-10 rounded-xl shadow-md'>
            <Image
                src="/check.png"
                alt="check"
                width={200}
            ...
            <div className='!mt-7 bg-white !p-5 rounded-lg shadow-md'>
                ...
                <div className='flex !gap-5 !mt-4 flex-wrap'>
                    <Button
                        className='!p-2 !px-4 !bg-blue-600 !text-white hover:!bg-blue-700 rounded-md shadow-sm transition'
                        onClick={onEmailShare}
                    >
                        <Mail className='!w-4 !h-4 !mr-2' /> Email
                    </Button>
                    <Button
                        className='!p-2 !px-4 !bg-green-600 !text-white hover:!bg-green-700 rounded-md shadow-sm transition'
                        onClick={onSmsShare}
                    >
                        <Smartphone className='!w-4 !h-4 !mr-2' /> SMS
                    </Button>
                    <Button
                        className='!p-2 !px-4 !bg-emerald-600 !text-white hover:!bg-emerald-700 rounded-md shadow-sm transition'
            ...
            <div className='flex !w-full !gap-5 justify-between !mt-6'>
                ...
                <Link href={'/dashboard/create-interview'}>
                    <Button className='group !p-2 !px-4 !mx-2 !my-2 !bg-blue-600 !text-white hover:!bg-blue-700 rounded-md shadow-md'>
                        <Plus className='plus-rotate !w-4 !h-4 !mr-2' /> Create New Interview
                    </Button>
                </Link>
            </div>
